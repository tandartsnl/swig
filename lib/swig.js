...
function getCACHE(cacheprefix) {
    if (cacheprefix) {
        if (!PREFIXEDCACHE[ cacheprefix ])
            PREFIXEDCACHE[ cacheprefix ] = {};

        return PREFIXEDCACHE[ cacheprefix ];
    }
...
function createTemplate(data, id) {
  var template = {
      ...
      compileFile: function(filepath, forceAllowErrors) { return exports.compileFile(filepath, forceAllowErrors); },
...
function getTemplate(source, options, cacheprefix) {
  var cache, key = options.filename || source;

  if (_config.cache || options.cache) {
    cache = getCACHE(cacheprefix);
...
exports.compileFile = function (filepath, forceAllowErrors, cacheprefix) {
  var tpl, get, cache;
  ...
  get = function () {
    ...
    getSingle = function (prefix) {
      ...
      try {
        ...
        tpl = getTemplate(data, { filename: filepath }, cacheprefix);
      } catch (e) {
        excp = e;
      }
    };
...
exports.compile = function (source, options, cacheprefix) {
  var tmpl = getTemplate(source, options || {}, cacheprefix);
